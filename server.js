...
// Setup multer for file uploads, limiting file size to 2MB
const upload = multer({
    limits: {
...
io.on('connection', (socket) => {
    ...
    socket.on('disconnect', () => {
        ...
        // Check if no users are left and reset the world state
        if (Object.keys(users).length === 0) {
            console.log('Last user disconnected. Resetting world state.');
            billboardImages = { '1': null, '2': null, '3': null, '4': null };
            chatMessages = [];
            io.emit('billboard-updated', { billboardId: '1', imageData: null });
            io.emit('billboard-updated', { billboardId: '2', imageData: null });
            io.emit('billboard-updated', { billboardId: '3', imageData: null });
            io.emit('billboard-updated', { billboardId: '4', imageData: null });
        }
    });
});
